...
def plot_snap(i0=-1):
    ...
    for i in [i0]:
        ...
        if lake_name != 'synth' and lake_name != 'nonlinear':
            ...
            label = r'altimetry'
            dV_t = dV_true*(H**2)/1e9

        plt.plot(t0,dV_alt,color='seagreen',linewidth=4,linestyle='-.',label=label)
        plt.plot(t0,dV_mean,color='indigo',linewidth=4,label=r'inversion')

        if lake_name == 'synth' or lake_name == 'nonlinear':
            plt.plot(t0,dV_t,color='k',linestyle=':',linewidth=8,label=r'true solution')
